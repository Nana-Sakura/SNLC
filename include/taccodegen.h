...
public:
    ...
    void registerProcParamOrder(const std::string& name,
    ...
    // 统计：spill 次数
    int spillCount() const { return totalSpills_; }
...
private:
    ...
    void emitLine(const std::string& s)      { out_ << "    " << s << "\n"; }
    void emitLbl(const std::string& l)       { out_ << l << ":\n"; }
    void emitCmt(const std::string& c)       { out_ << "    # " << c << "\n"; }
    void emitData(const std::string& s)      { data_ << s << "\n"; }
    ...
    void writeback(const std::string& vreg,
                   const std::string& tmp = "$t8");

    // 生成一个过程/main 的汇编
    void emitProc(const std::string& procLabel,
                  int localBytes,
    ...
    // 帧大小计算
    static int frameSize(int localBytes, int extraSpill) {
