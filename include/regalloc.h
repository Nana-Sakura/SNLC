...
struct InterferenceGraph {
    ...
    void addNode(const std::string& a) {
        adj.emplace(a, std::unordered_set<std::string>{});
    }
    int degree(const std::string& a) const {
        auto it = adj.find(a);
...
private:
    ...
    colorGraph(InterferenceGraph& ig,
               const std::vector<std::string>& vregs);

    // 判断是否是虚拟寄存器（以 % 开头）
    static bool isVReg(const std::string& s) {
        return !s.empty() && s[0] == '%';
    }
