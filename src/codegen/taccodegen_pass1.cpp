...
std::string TACCodeGen::newVReg() {
    return "%" + std::to_string(vregCtr_++);
}
std::string TACCodeGen::newLabel(const std::string& hint) {
    return hint + "_" + std::to_string(labelCtr_++);
}

void TACCodeGen::newBlock(const std::string& label) {
    blocks_.emplace_back();
    blocks_.back().label = label;
    curBB_ = &blocks_.back();
}

void TACCodeGen::emit(TACInstr instr) {
    if (!curBB_) newBlock();
    curBB_->instrs.push_back(std::move(instr));
}

// ─── 控制流连接：扫描 JUMP / BRANCH_EQ0 / LABEL，建立 succ/pred ──────────────
void TACCodeGen::linkBlocks() {
    // 建立 label → block 映射
    std::unordered_map<std::string, BasicBlock*> labelMap;
    for (auto& bb : blocks_)
        if (!bb.label.empty()) labelMap[bb.label] = &bb;
    ...
    for (int i = 0; i < (int)blocks_.size(); ++i) {
        ...
        if (last.op == TACOp::JUMP) {
            ...
            if (it != labelMap.end()) link(&bb, it->second);
        } else if (last.op == TACOp::BRANCH_EQ0) {
            // fall-through 到下一个块
            if (i + 1 < (int)blocks_.size()) link(&bb, &blocks_[i + 1]);
            // 跳转目标
            auto it = labelMap.find(last.label);
            if (it != labelMap.end()) link(&bb, it->second);
...
std::string TACCodeGen::generate(ASTNode* root) {
    emitData(".data");
...
// ── Program ───────────────────────────────────────────────────────────────────
void TACCodeGen::tacProgram(ASTNode* node) {
    ...
    // main
    blocks_.clear(); vregCtr_ = 0;
    newBlock("main");
    Scope* saved = current_; current_ = global_;
...
// ── ProcDec ───────────────────────────────────────────────────────────────────
void TACCodeGen::tacProcDec(ASTNode* node) {
    if (!node) return;
    for (auto& proc : node->children) {
        ...
        if (pit != procParamOrder_.end()) {
            ...
            for (Symbol* sym : pit->second) {
                ...
                emit(TACInstr::make(TACOp::STORE, "",
                                    ar[ai], "$fp", sym->offset,
                                    "# save_arg " + sym->name));
                ++ai;
            }
...
// ── StmList ───────────────────────────────────────────────────────────────────
void TACCodeGen::tacStmList(ASTNode* node) {
    if (!node) return;
    for (auto& stm : node->children) tacStm(stm.get());
}

// ── Stm ───────────────────────────────────────────────────────────────────────
void TACCodeGen::tacStm(ASTNode* node) {
    if (!node) return;
    if (node->name == "__block__") { tacStmList(node); return; }
    ...
    switch (node->kind) {
        ...
        case NodeKind::CALL_STM: {
            ...
            emit(TACInstr::make(TACOp::CALL, "", node->name, "", (int)node->children.size()));
            break;
        }
        ...
        default:
            for (auto& c : node->children) tacStm(c.get());
            break;
    }
}

// ── 表达式 → vreg ─────────────────────────────────────────────────────────────
std::string TACCodeGen::tacExp(ASTNode* node) {
    if (!node) return "%zero";
    ...
    switch (node->kind) {
        ...
        case NodeKind::BINARY_EXP: {
            if (node->children.size() < 2) return newVReg();
            std::string l = tacExp(node->children[0].get());
            std::string r = tacExp(node->children[1].get());
            std::string d = newVReg();
            TACOp op;
            const std::string& name = node->name;
            if      (name == "+") op = TACOp::ADD;
            else if (name == "-") op = TACOp::SUB;
            else if (name == "*") op = TACOp::MUL;
            else if (name == "/") op = TACOp::DIV;
            else if (name == "<") op = TACOp::SLT;
            else if (name == "=") op = TACOp::SEQ;
            else op = TACOp::ADD;
        ...
        case NodeKind::VAR_EXP:
            if (!node->children.empty()) return tacExp(node->children[0].get());
            return newVReg();
...
std::string TACCodeGen::tacRelExp(ASTNode* node) {
    return tacExp(node);
}

// ── 读变量值 ──────────────────────────────────────────────────────────────────
std::string TACCodeGen::tacVarLoad(ASTNode* node) {
    if (!node) return newVReg();

    if (node->kind == NodeKind::SIMPLE_VAR) {
        Symbol* sym = current_->lookup(node->name);
        if (!sym) { auto d = newVReg(); emit(TACInstr::make(TACOp::LOAD_IMM,d,"","",0)); return d; }
        std::string d = newVReg();
        if (sym->isVarParam) {
            // 先取指针，再解引用
...
// ── 写变量 ────────────────────────────────────────────────────────────────────
void TACCodeGen::tacVarStore(ASTNode* node, const std::string& src) {
    if (!node) return;
    // 跳过 VAR_EXP 包装
...
// ── 取变量地址（VAR 参数传递用）─────────────────────────────────────────────
void TACCodeGen::tacLoadAddr(ASTNode* node, const std::string& dst) {
    if (!node) return;
    ASTNode* inner = (node->kind == NodeKind::VAR_EXP && !node->children.empty())
                     ? node->children[0].get() : node;
    if (inner->kind == NodeKind::SIMPLE_VAR) {
...
// ── 数组地址计算 ──────────────────────────────────────────────────────────────
std::string TACCodeGen::tacIndexAddr(ASTNode* node) {
    if (node->children.size() < 2) return newVReg();
    ...
    Symbol* sym = b->kind == NodeKind::SIMPLE_VAR ? current_->lookup(b->name) : nullptr;
    ...
    if (sym) {
        if (sym->isVarParam) {
            ...
            emit(TACInstr::make(TACOp::LOAD_ADDR, baseAddr, "$fp", "", sym->offset));
        }
    } else {
    ...
    // index - low
    if (sym && sym->type && sym->type->kind == TypeKind::ARRAY && sym->type->low != 0) {
...
// ── record 字段地址计算 ───────────────────────────────────────────────────────
std::string TACCodeGen::tacFieldAddr(ASTNode* node) {
    if (node->children.empty()) return newVReg();
    ASTNode* base = node->children[0].get();
    ASTNode* b    = (base->kind == NodeKind::VAR_EXP && !base->children.empty())
                    ? base->children[0].get() : base;
    Symbol* sym = b->kind == NodeKind::SIMPLE_VAR ? current_->lookup(b->name) : nullptr;
