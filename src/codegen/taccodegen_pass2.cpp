...
void TACCodeGen::emitProc(const std::string& procLabel,
    ...
    // 约定：spill 槽 offset 直接使用 allocation_.spillSlots[v]（已经是相对 $fp 的负值）
    // 为了不与局部变量重叠，把 spill 槽放在 localBytes 之后：
...
// ─── prologue / epilogue ──────────────────────────────────────────────────────
void TACCodeGen::emitPrologue(int fsize) {
...
void TACCodeGen::emitEpilogue(int fsize) {
    emitCmt("epilogue");
...
// ─── 虚拟寄存器解析 ───────────────────────────────────────────────────────────
// 如果 vreg 被分配了物理寄存器 → 直接返回物理寄存器名
// 如果 vreg 被 spill → 从栈槽 load 到 tmp，返回 tmp
// 如果 vreg 是物理寄存器（$v0, $a0...）或 $fp/$sp → 直接返回
std::string TACCodeGen::resolve(const std::string& vreg, bool load,
                                 const std::string& tmp) {
    if (vreg.empty() || vreg == "$zero") return "$zero";
    // 已经是物理寄存器
    ...
    if (sit != allocation_.spillSlots.end()) {
        ...
        if (load)
            emitLine("lw    " + tmp + ", " + std::to_string(off) + "($fp)  # spill load " + vreg);
        return tmp;
    }
    // 未分配（不应发生）
    return tmp;
...
void TACCodeGen::writeback(const std::string& vreg, const std::string& tmp) {
    if (vreg.empty() || vreg[0] == '$') return;
    auto cit = allocation_.coloring.find(vreg);
    if (cit != allocation_.coloring.end() && !cit->second.empty()) return; // 已在寄存器，无需回写

    auto sit = allocation_.spillSlots.find(vreg);
    if (sit != allocation_.spillSlots.end()) {
        int off = spillBase_ + sit->second;
        emitLine("sw    " + tmp + ", " + std::to_string(off) + "($fp)  # spill store " + vreg);
    }
}
...
// 逐条 TAC 指令发射
// ═══════════════════════════════════════════════════════════════════════════════
void TACCodeGen::emitInstr(const TACInstr& I) {
    switch (I.op) {
        ...
        case TACOp::SLT: case TACOp::SEQ: {
            ...
            switch (I.op) {
                ...
                case TACOp::SUB: emitLine("sub   " + d + ", " + l + ", " + r); break;
                case TACOp::MUL: emitLine("mul   " + d + ", " + l + ", " + r); break;
                case TACOp::DIV:
                    emitLine("div   " + l + ", " + r);
                    emitLine("mflo  " + d);
                    break;
