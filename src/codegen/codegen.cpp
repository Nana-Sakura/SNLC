...
CodeGen::CodeGen(Scope* globalScope)
    : global_(globalScope), current_(globalScope)
{
}

std::string
CodeGen::newLabel(const std::string& hint)
{
...
void
CodeGen::emit(const std::string& instr)
{
  out_ << "    " << instr << "\n";
}
void
CodeGen::emitLabel(const std::string& label)
{
  out_ << label << ":\n";
}
void
CodeGen::emitComment(const std::string& comment)
{
  out_ << "    # " << comment << "\n";
}
void
CodeGen::emitData(const std::string& line)
{
  data_ << line << "\n";
}
...
void
CodeGen::genProgram(ASTNode* node)
{
...
void
CodeGen::genProcDec(ASTNode* node)
{
  if(!node)
    return;
  for(auto& proc : node->children)
    {
      ...
      // 找到过程的子作用域：遍历 scopes_ 找 parent == current_
      // 且含有过程的局部变量 简化：推进 current_
      // 到下一个子作用域（深度优先顺序与语义分析一致） 这里用一个小技巧：保存
      // current_ 然后从全局重新找 更实用的方法：把 Scope* 存入
      // Symbol，语义分析阶段填充
      ...
      Scope* procScope = nullptr;
      {
        ...
        if(it2 != procScopeMap_.end())
          procScope = it2->second;
      }
...
void
CodeGen::genStmList(ASTNode* node)
{
  if(!node)
    return;
...
// ─────────────────────────────────────────────────────────────────────
void
CodeGen::genAssign(ASTNode* node)
{
...
CodeGen::genCall(ASTNode* node)
{
  ...
  std::vector<Symbol*> paramSyms;
  {
    ...
    if(it2 != procParamOrder_.end())
      paramSyms = it2->second;
  }
  ...
  for(auto& arg : node->children)
    {
      ...
      if(isVarArg)
      ...
      else
        {
          ...
          if(i < 4 && val != dest)
            emit("move  " + std::string(argRegs[i]) + ", " + val);
        }
...
// ───────────────────────────────────────────────────────────────────
void
CodeGen::genReturn(ASTNode* node)
{
  emitComment("return");
  if(!node->children.empty())
    {
...
// ═══════════════════════════════════════════════════════════════════════════════
void
CodeGen::pushReg(const std::string& r)
{
  emit("addiu $sp, $sp, -4");
  emit("sw    " + r + ", 0($sp)");
}
void
CodeGen::popReg(const std::string& r)
{
  emit("lw    " + r + ", 0($sp)");
  emit("addiu $sp, $sp, 4");
}
...
//
std::string
CodeGen::genExp(ASTNode* node, const std::string& dest)
{
  if(!node)
    return "$zero";
...
std::string
CodeGen::genRelExp(ASTNode* node, const std::string& dest)
{
  if(!node)
    return "$zero";
  std::string d = dest.empty() ? allocTemp() : dest;
...
std::string
CodeGen::genVarAddr(ASTNode* node)
{
  if(!node)
    return "0($fp)";
...
void
CodeGen::genStore(ASTNode* lhs, const std::string& val)
{
  if(!lhs)
    return;
  // VAR_EXP 包装层
...
void
CodeGen::genLoadAddr(ASTNode* node, const std::string& dest)
{
  if(!node)
    return;
...
//  $t9 用作临时 index 计算
void
CodeGen::genIndexAddr(ASTNode* node)
{
  if(node->children.size() < 2)
    return;
...
// ────────────────────────────────────────────────────
void
CodeGen::genFieldAddr(ASTNode* node)
{
  if(node->children.empty())
    return;
  ASTNode* base = node->children[0].get();
