...
void Optimizer::optimize(ASTNode* root, Scope* scope) {
    if (!root) return;
...
bool Optimizer::isConst(const ASTNode* node, int& val) const {
    ...
    if (node->kind == NodeKind::INT_LITERAL) { val = node->ival; return true; }
    return false;
}
...
ASTPtr Optimizer::tryFold(ASTNode* node) {
    ...
    else if (op == "/") { if (rv == 0) return nullptr; result = lv / rv; }
...
bool Optimizer::trySimplify(ASTPtr& node) {
    ...
    if (op == "+") {
        if (Rc && rv == 0) { node = std::move(node->children[0]); return true; }
        if (Lc && lv == 0) { node = std::move(node->children[1]); return true; }
    }
    if (op == "-") {
        if (Rc && rv == 0) { node = std::move(node->children[0]); return true; }
    }
    if (op == "*") {
        if (Rc && rv == 1) { node = std::move(node->children[0]); return true; }
        if (Lc && lv == 1) { node = std::move(node->children[1]); return true; }
        if (Rc && rv == 0) { node = makeInt(0, node->line); return true; }
        if (Lc && lv == 0) { node = makeInt(0, node->line); return true; }
    }
    if (op == "/") {
        if (Rc && rv == 1) { node = std::move(node->children[0]); return true; }
    }
...
std::string Optimizer::newTmpName() {
    return "__licm_" + std::to_string(tmpCounter_++);
}

// 构造 "tmpName := expr" 的 ASSIGN_STM 节点
ASTPtr Optimizer::makeAssign(const std::string& tmpName, ASTPtr expr, int line) {
...
// 构造 SIMPLE_VAR 节点
ASTPtr Optimizer::makeVar(const std::string& name, int line) {
...
void Optimizer::licmPass(ASTNode* root) {
    if (!root) return;
...
// siblings：包含该 while 的 children 列表
// idx：while 在 siblings 中的当前下标
void Optimizer::licmVisit(ASTList& siblings, int idx) {
    ...
    for (auto& stm : body->children) {
        ...
        if (stm && stm->kind == NodeKind::ASSIGN_STM
                && stm->children.size() >= 2) {
            findInvariants(stm->children[1], defs, hoisted, extracted);
        }
        // write 语句的表达式也可以提取
        if (stm && stm->kind == NodeKind::WRITE_STM
                && !stm->children.empty()) {
            findInvariants(stm->children[0], defs, hoisted, extracted);
        }
        // if/while 条件里也可以提取（但嵌套循环已由递归处理，这里只看条件）
        if (stm && stm->kind == NodeKind::IF_STM
                && !stm->children.empty()) {
            findInvariants(stm->children[0], defs, hoisted, extracted);
        }
    }

    if (extracted.empty()) return; // 没有找到不变量，不做任何修改
    ...
    for (int k = (int)extracted.size() - 1; k >= 0; --k) {
        ...
        siblings.insert(siblings.begin() + idx,
                        std::move(assignStm));
...
// ── 收集循环体内所有被赋值的变量名 ───────────────────────────────────────────
void Optimizer::collectDefs(const ASTNode* node,
    ...
    if (node->kind == NodeKind::ASSIGN_STM && node->children.size() >= 1) {
        ...
        if (lhs) {
            ...
            if (lhs->kind == NodeKind::INDEX_VAR
                    && !lhs->children.empty()
                    && lhs->children[0])
                defs.insert(lhs->children[0]->name);
            // record 字段 p.x := ...：记录 p 被写了
            if (lhs->kind == NodeKind::FIELD_VAR
                    && !lhs->children.empty()
                    && lhs->children[0])
                defs.insert(lhs->children[0]->name);
        }
...
// ── 判断表达式子树是否是循环不变量 ───────────────────────────────────────────
bool Optimizer::isInvariant(const ASTNode* node,
