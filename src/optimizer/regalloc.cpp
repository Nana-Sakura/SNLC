...
// ─── TACInstr 工厂 ────────────────────────────────────────────────────────────
TACInstr TACInstr::make(TACOp op,
...
// ─── def / use 集合 ───────────────────────────────────────────────────────────
std::vector<std::string> RegAllocator::getDefs(const TACInstr& i) {
    std::vector<std::string> d;
    if (!i.dst.empty() && isVReg(i.dst)) d.push_back(i.dst);
    return d;
}

std::vector<std::string> RegAllocator::getUses(const TACInstr& i) {
    std::vector<std::string> u;
    auto add = [&](const std::string& s){ if (isVReg(s)) u.push_back(s); };
    switch (i.op) {
        ...
        case TACOp::PARAM: case TACOp::RETURN:
            add(i.src1); break;
        case TACOp::LOAD:
            add(i.src1); break;   // base
        case TACOp::STORE:
            add(i.src1); add(i.src2); break; // src, base
...
// 活跃变量分析（经典逆序数据流）
// ═══════════════════════════════════════════════════════════════════════════════
void RegAllocator::livenessAnalysis(std::vector<BasicBlock>& blocks) {
    // 1. 计算每个基本块的局部 def/use 集合
    for (auto& bb : blocks) {
        bb.def.clear(); bb.use.clear();
        for (auto& instr : bb.instrs) {
            // use：在本块中先被使用（未被前面指令 def）的变量
            for (auto& u : getUses(instr))
                if (!bb.def.count(u)) bb.use.insert(u);
            // def：本块中定义的变量
    ...
    while (changed) {
        ...
        for (int i = (int)blocks.size() - 1; i >= 0; --i) {
            ...
            for (auto* succ : bb.succs)
                for (auto& v : succ->liveIn) newOut.insert(v);
...
// 构建干涉图
// ═══════════════════════════════════════════════════════════════════════════════
InterferenceGraph RegAllocator::buildInterferenceGraph(
    ...
    for (auto& bb : blocks) {
        ...
        for (int i = (int)bb.instrs.size() - 1; i >= 0; --i) {
            ...
            // 确保所有相关虚拟寄存器都在图中
            for (auto& v : live)  ig.addNode(v);
            for (auto& d : defs)  ig.addNode(d);
            for (auto& u : uses)  ig.addNode(u);
...
RegAllocator::colorGraph(InterferenceGraph& ig,
    ...
    if (vregs.empty()) return coloring;
        ...
        for (auto& v : remaining) {
            ...
            if (it != adjCopy.end())
                for (auto& nb : it->second)
                    if (remaining.count(nb)) ++deg;
            if (deg < K) { chosen = v; break; }
        }
        ...
        if (chosen.empty()) {
            ...
            for (auto& v : remaining) {
                ...
                if (it != adjCopy.end())
                    for (auto& nb : it->second)
                        if (remaining.count(nb)) ++deg;
                if (deg > maxDeg) { maxDeg = deg; chosen = v; }
            }
        }
    ...
    while (!stk.empty()) {
        ...
        for (int k = 0; k < K; ++k) {
            ...
            if (!usedColors.count(pr)) { assigned = pr; break; }
        }
...
// 主分配入口
// ═══════════════════════════════════════════════════════════════════════════════
RegAllocator::Result RegAllocator::allocate(std::vector<BasicBlock>& blocks) {
    Result result;
    ...
    std::vector<std::string> vregs;
    {
        ...
        for (auto& bb : blocks)
            for (auto& instr : bb.instrs) {
                for (auto& d : getDefs(instr)) if (seen.insert(d).second) vregs.push_back(d);
                for (auto& u : getUses(instr)) if (seen.insert(u).second) vregs.push_back(u);
            }
    }
    if (vregs.empty()) return result;
