...
std::shared_ptr<TypeDesc> SemanticAnalyzer::buildTypeDesc(ASTNode* node) {
...
int SemanticAnalyzer::typeSize(const TypeDesc* td) {
    if (!td) return 4;
    switch (td->kind) {
        case TypeKind::INTEGER: return 4;
        case TypeKind::CHAR:    return 4;
        case TypeKind::NAME: {
            auto* sym = current_->lookup(td->name);
            return sym && sym->kind == SymKind::TYPE ? typeSize(sym->type.get()) : 4;
        }
        case TypeKind::ARRAY: {
        ...
        case TypeKind::RECORD: {
            int total = 0;
            for (auto& f : td->fields) total += typeSize(f.type.get());
            return total;
        }
    }
    return 4;
...
void SemanticAnalyzer::registerIds(ASTNode* entryNode,
...
void SemanticAnalyzer::visitTypeDec(ASTNode* node) {
    if (!node) return;
    for (auto& listNode : node->children) {          // TypeDecList
        for (auto& child : listNode->children) {     // 单条别名：name=别名, children[0]=类型
            if (child->children.empty()) continue;
            auto type = buildTypeDesc(child->children[0].get());
            if (!type) { error("无法解析类型：" + child->name, child->line); continue; }
...
void SemanticAnalyzer::visitVarDec(ASTNode* node) {
    ...
    for (auto& entry : node->children) {
        ...
        if (!type) { error("无法解析变量类型", entry->line); continue; }
    ...
    for (auto& proc : node->children) {
        ...
        if (!proc->children.empty()) {
            ...
            for (auto& param : params->children) {
                if (param->children.empty()) continue;
        ...
        // 局部声明
        if (proc->children.size() > 1) visitDeclarePart(proc->children[1].get());
        // 过程体语句
        if (proc->children.size() > 2) visitStmList(proc->children[2].get());
...
void SemanticAnalyzer::visitStm(ASTNode* node) {
    ...
    // block 容器
    if (node->name == "__block__") { visitStmList(node); return; }
    ...
    switch (node->kind) {
        ...
        default:
            for (auto& c : node->children) visitStm(c.get());
            break;
    }
}
...
std::shared_ptr<TypeDesc> SemanticAnalyzer::visitExp(ASTNode* node) {
    ...
    switch (node->kind) {
        ...
        default:
            for (auto& c : node->children) visitExp(c.get());
            return nullptr;
    }
}
...
std::shared_ptr<TypeDesc> SemanticAnalyzer::visitVariable(ASTNode* node) {
    ...
    if (node->kind == NodeKind::INDEX_VAR) {
        auto baseType = !node->children.empty() ? visitVariable(node->children[0].get()) : nullptr;
        auto idxType  = node->children.size() > 1 ? visitExp(node->children[1].get()) : nullptr;
        ...
        for (auto& f : baseType->fields) {
            if (f.name == node->name) {
                ...
                if (node->children.size() > 1) {
                    ...
                    if (!node->typeInfo || node->typeInfo->kind != TypeKind::ARRAY)
...
bool SemanticAnalyzer::typeCompatible(const TypeDesc* lhs, const TypeDesc* rhs) {
    ...
    if (lhs->kind == TypeKind::RECORD) {
        ...
        for (size_t i = 0; i < lhs->fields.size(); ++i)
            if (!typeCompatible(lhs->fields[i].type.get(), rhs->fields[i].type.get()))
                return false;
    }
    return true;
}
...
std::shared_ptr<TypeDesc> SemanticAnalyzer::resolveType(const TypeDesc* td) {
    if (!td || td->kind != TypeKind::NAME) return nullptr;
    auto* sym = current_->lookup(td->name);
...
static const char* symKindName(SymKind k) {
    switch (k) {
        ...
        case SymKind::VAR_PARAM: return "var-param";
...
static std::string typeName(const TypeDesc* td) {
    if (!td) return "nil";
    switch (td->kind) {
        case TypeKind::INTEGER: return "integer";
        case TypeKind::CHAR:    return "char";
        case TypeKind::NAME:    return td->name;
        case TypeKind::ARRAY:
...
void SemanticAnalyzer::dumpScope(Scope* s, int indent) const {
    if (!s) return;
    std::string pad(indent * 2, ' ');
    for (auto& [name, sym] : s->table) {
        std::cout << pad
                  << symKindName(sym.kind) << "  " << name
