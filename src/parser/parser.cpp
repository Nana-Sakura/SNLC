...
ASTPtr Parser::parseProgramHead() {
...
ASTPtr Parser::parseDeclarePart() {
...
ASTPtr Parser::parseProgramBody() {
    int line = peek().line;
...
ASTPtr Parser::parseTypeDecList() {
    auto node = makeNode(NodeKind::TYPE_DEC, peek().line);
    do {
...
ASTPtr Parser::parseStructureType() {
...
ASTPtr Parser::parseRecType() {
    ...
    while (!check(TokenType::KW_END) && !check(TokenType::EOF_TOK)) {
        ASTPtr typeNode;
        if (check(TokenType::KW_ARRAY)) typeNode = parseArrayType();
        else typeNode = parseBaseType();
...
ASTPtr Parser::parseVarDec() {
    auto node = makeNode(NodeKind::VAR_DEC, peek().line);
    if (!check(TokenType::KW_VAR)) return node;
    advance(); // 消耗 VAR
...
void Parser::parseIdList(ASTNode* parent) {
    do {
...
void Parser::parseVarIdList(ASTNode* parent) {
...
ASTPtr Parser::parseParamList() {
    auto node = makeNode(NodeKind::VAR_DEC, peek().line);
    node->name = "__params__";
    if (check(TokenType::RPAREN)) return node; // 空参数列表
...
ASTPtr Parser::parseParam() {
    bool isVar = false;
    if (check(TokenType::KW_VAR)) { advance(); isVar = true; }
    auto typeNode = parseTypeName();
...
ASTPtr Parser::parseStm() {
    switch (peek().type) {
        case TokenType::KW_IF:     return parseConditionalStm();
        case TokenType::KW_WHILE:  return parseLoopStm();
        case TokenType::KW_READ:   return parseInputStm();
        case TokenType::KW_WRITE:  return parseOutputStm();
        case TokenType::KW_RETURN: return parseReturnStm();
        case TokenType::ID:        return parseAssignOrCall();
        default:
...
ASTPtr Parser::parseAssignOrCall() {
...
ASTPtr Parser::parseAssignmentRest(const std::string& id, int line) {
...
ASTPtr Parser::parseCallStmRest(const std::string& id, int line) {
    expect(TokenType::LPAREN, "期望 '('");
...
ASTPtr Parser::parseConditionalStm() {
    int line = peek().line;
...
ASTPtr Parser::parseRelExp() {
    ...
    // OtherRelE ::= CmpOp Exp
    std::string op;
    if (check(TokenType::LT)) { op = "<"; advance(); }
    else if (check(TokenType::EQ)) { op = "="; advance(); }
    else {
        syncError("期望比较运算符 '<' 或 '='");
        return left;
    }
...
ASTPtr Parser::parseVariMore(ASTPtr base) {
